...
/**
 * Detect Chrome installation on macOS and other platforms
 */
function detectChromePath(): string | null {
  ...
  // Check if running in production/Docker environment - primary Chrome
  if (process.env.CHROME_PATH) {
    try {
      ...
      console.warn(
        "Primary Chrome path not accessible:",
  ...
  // Check fallback path for Docker (Chromium)
  if (process.env.CHROME_PATH_FALLBACK) {
    try {
      if (fs.existsSync(process.env.CHROME_PATH_FALLBACK)) {
        console.log(
          "Using fallback Chrome path:",
          process.env.CHROME_PATH_FALLBACK
        );
        return process.env.CHROME_PATH_FALLBACK;
      }
    } catch (e) {
      console.warn(
        "Fallback Chrome path not accessible:",
        process.env.CHROME_PATH_FALLBACK,
        e
      );
    }
  ...
  // For development, try to find Chrome
...
function extractMetrics(
  lhr: any
): Omit<AuditResult, "url" | "status" | "error"> {
...
async function auditURL(
...
): Promise<Omit<AuditResult, "url" | "status" | "error">> {
  ...
  if (!chromePath) {
    throw new Error(
      "No Chrome installations found. Please install Google Chrome or Chromium. " +
        "For macOS: Install Chrome from https://www.google.com/chrome/. " +
        "For development with Docker: Use the Dockerfile.dev.chrome to run with Chrome."
    );
  }
  ...
  // Launch Chrome with detected path, flags, and host for Docker/K8s
  ...
  try {
    for (let i = 0; i < numberOfRuns; i++) {
      logger.info(
        `Starting Lighthouse run ${i + 1}/${numberOfRuns} for ${url}`
      );
      ...
      runs.push(metrics);
      logger.info(
        `Finished Lighthouse run ${i + 1}/${numberOfRuns} in ${(
          (Date.now() - runStart) /
          1000
        ).toFixed(1)}s`
      );
  ...
  logger.info(
    `auditURL for ${url} took ${((Date.now() - auditStart) / 1000).toFixed(2)}s`
  );
